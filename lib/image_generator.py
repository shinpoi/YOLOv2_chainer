...
def random_overlay_image(src_image, overlay_image, minimum_crop):
    ...
    y = int(np.random.randint((src_h-scale_item_h) if (src_h-scale_item_h) > 1 else 2) - shift_item_h)
    x = int(np.random.randint((src_w-scale_item_w) if (src_w-scale_item_w) > 1 else 2) - shift_item_w)
